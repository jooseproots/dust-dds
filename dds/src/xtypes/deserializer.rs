...
trait XTypesDeserialize {
    ...
    fn deserialize_array(
        ...
        match sequence_type.get_kind() {
            ...
            TypeKind::STRING8 => dynamic_data.set_sequence_values::<String>(
                member.get_id(),
                self.deserialize_string_array(bound)?,
            ),
            TypeKind::STRING16 => todo!(),
            TypeKind::ALIAS => todo!(),
            TypeKind::ENUM => dynamic_data.set_sequence_values::<DynamicData>(
                member.get_id(),
    ...
    fn deserialize_string_array(&mut self, length: u32) -> XTypesResult<Vec<String>> {
        let mut sequence_of_string = Vec::with_capacity(length as usize);
        for _ in 0..length {
    ...
    fn deserialize_complex_array(
        ...
        let mut sequence_of_dynamic_data = Vec::with_capacity(length as usize);
        for _ in 0..length {
    ...
    fn deserialize_mutable_struct(
        ...
        for member_index in 0..dynamic_type.get_member_count() {
            ...
            if member_descriptor.r#type.get_kind() == TypeKind::SEQUENCE {
                ...
                dynamic_data.set_sequence_values::<DynamicData>(pid as u32, sequence)?;
